...
def write_web_data_js(card: dict[str, Any], out_paths: list[Path]) -> list[Path]:
    """Embed website_card into static data.js so the site works without the API."""
    payload = json.dumps(card, indent=2, ensure_ascii=False)
    text = (
        "/* Auto-generated by scripts/export_portfolio.py — do not edit by hand */\n"
        f"window.POLICYSHIFT_CARD = {payload};\n"
    )
    written: list[Path] = []
...
def build_embedded_experiments(artifact_root: str | Path = "artifacts/experiments") -> dict[str, Any]:
    """Slim experiment snapshots for static playback (no live API required)."""
    root = Path(artifact_root)
    ...
    for exp in sorted(root.iterdir()):
        ...
        for name in (
            ...
            if path.exists():
                try:
                    item[name.replace(".json", "")] = json.loads(path.read_text(encoding="utf-8"))
                except json.JSONDecodeError:
                    continue
        summary = exp / "metrics" / "summary.json"
        if summary.exists():
            try:
                item["summary"] = json.loads(summary.read_text(encoding="utf-8"))
            except json.JSONDecodeError:
                pass
        traj_path = exp / "traces" / "trajectories.jsonl"
        trajs: list[Any] = []
        if traj_path.exists():
        ...
        item["sample_trajectories"] = trajs
        # Bound payload size for huge summaries
        for key, value in list(item.items()):
            raw = json.dumps(value, default=str)
            if len(raw) > 200_000 and key not in {"id", "sample_trajectories"}:
                item[key] = {
                    "_truncated": True,
                    "n_bytes": len(raw),
                    "keys": list(value) if isinstance(value, dict) else type(value).__name__,
                }
        experiments.append(item)
    return {"experiments": experiments}


def write_experiments_js(
    payload: dict[str, Any],
    out_paths: list[Path],
) -> list[Path]:
    text = (
        "/* Auto-generated by scripts/export_portfolio.py — do not edit by hand */\n"
        f"window.POLICYSHIFT_EXPERIMENTS = {json.dumps(payload, default=str)};\n"
    )
    written: list[Path] = []
    for path in out_paths:
        path.parent.mkdir(parents=True, exist_ok=True)
...
def write_portfolio_export(
    ...
    *,
    ...
    # Keep static site(s) in sync for GitHub Pages / local file open
    import shutil

    web_roots = [Path("apps/web"), Path("docs/site")]
    for web_root in web_roots:
        ensure_dir(web_root)
        for name in ("index.html", "styles.css", "app.js"):
            src = Path("apps/web") / name
            if src.exists() and web_root != Path("apps/web"):
                shutil.copy2(src, web_root / name)
        (web_root / ".nojekyll").write_text("", encoding="utf-8")
    data_js_paths = write_web_data_js(
        website,
        [Path("apps/web/data.js"), Path("docs/site/data.js")],
    )
    for i, p in enumerate(data_js_paths):
        paths[f"web_data_{i}"] = p
    exp_payload = build_embedded_experiments(artifact_root)
    exp_js_paths = write_experiments_js(
        exp_payload,
        [Path("apps/web/experiments.js"), Path("docs/site/experiments.js")],
    )
    for i, p in enumerate(exp_js_paths):
        paths[f"web_experiments_{i}"] = p
    return paths
